...
export class RestApiService {
  ...
  getEmployees(): Observable<Paciente> {
    return this.http
      .get<Paciente>(this.apiURL + '/pacientes')
      .pipe(retry(1), catchError(this.handleError));
  }
  // HttpClient API get() method => Fetch employee
  getEmployee(id: any): Observable<Paciente> {
    return this.http
      .get<Paciente>(this.apiURL + '/pacientes/' + id)
      .pipe(retry(1), catchError(this.handleError));
  }
  // HttpClient API post() method => Create employee
  createEmployee(employee: any): Observable<Paciente> {
    return this.http
      .post<Paciente>(
  ...
  updateEmployee(id: any, employee: any): Observable<Paciente> {
    return this.http
      .put<Paciente>(
